...
def click_login_Btn(): #is_loginOK=True # 데모용 모든 조건 True로 설정
    ...
    robot_ip=tmp[1].split("=")[1]

    # 전역 변수 저장
    page_status["login_part"]["is_loginOK"]=True
    page_status["login_part"]["User_ID"]=user_id
    page_status["login_part"]["Robot_IP"]=robot_ip

    return True, user_id, robot_ip
...
def click_logout_Btn():
    ...
    page_status["login_part"]["Robot_IP"]=""
    user_id=""
    robot_ip=""
    
    return False, user_id, robot_ip

@bp.route("/User/<user_btn>/",methods=['POST'])
def click_user_btn(user_btn):
    if user_btn=="Login":
        is_loginOK,user_id, robot_ip=click_login_Btn()
    elif user_btn=="Logout":
        is_loginOK,user_id, robot_ip=click_logout_Btn()
      
    #response=make_response(render_template('index.html',is_loginOK=is_loginOK))
    #response.mimetype="text/html"
    return render_template('index.html',is_loginOK=is_loginOK,user_id=user_id, robot_ip=robot_ip)
